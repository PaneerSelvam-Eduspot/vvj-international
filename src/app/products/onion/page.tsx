...
export const metadata: Metadata = { title: "Onion Export" };

export default function OnionPage() {
  const product = getProductBySlug("onion")!;
  return (
    <>
      ...
      <section className="section-padding bg-bg">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-16">
          <div>
            <div className="relative h-64 mb-8 overflow-hidden border border-vvj-border">
              <Image
                src={product.image}
                alt="Premium onion produce for export"
                fill
            ...
            <ul className="space-y-3">
              {product.highlights.map((h) => (
                <li key={h} className="flex items-start gap-3 text-sm text-vvj-muted">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-mid flex-shrink-0 mt-2" />
                  {h}
                </li>
              ))}
            </ul>
          ...
          <div className="p-8 border border-emerald-border bg-surface h-fit">
            ...
            <p className="text-sm text-vvj-muted leading-relaxed mb-6">
              Send us your target quantity, destination port, and preferred
              packaging — we&apos;ll respond within 24 hours with pricing and
              availability.
            </p>
            <a
              href="/contact"
